...
状态：P0 已完成；P1 文档已交付、目标环境验收待执行；P2 本地预检通过、远端 CI 待运行 · 日期：2026-09-24 · 基线提交：`b37d09a`

本方案依据当前[项目分析报告](../PROJECT_ANALYSIS.md)、[发布前审查](release-readiness-audit-20260916.md)、`internal/server/register.go` 和现有 CI 配置制定。P0 的设计与验收结果、P1 的文档交付及 P2 的本地预检保留在本文；远端 CI 与目标环境验收尚未完成。前一份路线图中的熔断器、Prometheus 指标和工具前缀通配符现已存在，不再列为待开发能力。
...
本地构建和原生产物冒烟已通过，具体证据见[发布构建验证记录](release-validation-2026-09-24.md)。最终验收仍要求同一提交上的现有 CI 门禁通过，并由 Linux CI 对产物完成 MCP 初始化及 `tools/list` 冒烟。真实上游联调仍应使用显式凭据与独立环境，不放入普通 PR 门禁。回滚时使用上一版已验证产物和对应配置。

## 执行顺序与完成标准

P0 已完成，P1 指南与检查清单已交付，P2 构建与本地预检已完成。目标环境的启动、`tools/list` 和探针验收仍需在部署时完成；P2 还需以最终提交触发并核对远端 CI。各阶段分别评审，避免部署文档和 CI 改动混在一个变更中。完成本阶段的判定条件是：纯文件工具组合无需上游令牌即可启动；部署指南能覆盖主要运行方式与安全边界；CI 构建与正式发布命令一致且有产物冒烟验证记录。
